(function(root,factory){
  ...
  function normalizeFundamentals(raw={},stock={}){
    ...
    const publicationDates=annual.map(x=>x.accepted||x.filed||x.publishedDate||x.availableFrom).filter(Boolean).map(String).sort();
    return {symbol:stock.s||stock.symbol||raw.symbol||null,name:raw.name||stock.n||null,annual,source:raw.source||'fundamentals-adapter',status:raw.status||'ok',warning:raw.warning||null,providerCode:raw.providerCode||null,
      asOf:publicationDates.at(-1)||null};
  }
  function buildSnapshot(stock={},market={},fundamentals={},core){
    if(!core)throw new Error('JuKaCore wird benötigt');
    const diagnostics=[];
    const safe=(label,fn,fallback=null)=>{try{return fn()}catch(e){diagnostics.push({stage:label,message:e?.message||String(e),code:e?.code||'CALCULATION_ERROR'});return fallback}};
    const m=normalizeMarket(market,stock),f=normalizeFundamentals(fundamentals,stock);
    const derived=safe('derive-fundamentals',()=>core.deriveFundamentals(f.annual),[])||[];
    const latest=derived.at(-1)||null,price=m.prices.at(-1)?.close??null;
    const model=safe('classify-model',()=>core.classifyValuationModel(stock),'operating-company')||'operating-company';
    const fallbackReadiness={ready:false,label:'Bewertung nicht verfügbar',missingRequired:['valuation-engine'],valuationEngine:'unavailable'};
    const engine=safe('valuation-engine',()=>core.jukaValuationEngine(stock,derived,price,stock.overrides||{}),{readiness:fallbackReadiness,quality:null,valuation:null,relative:null,reverse:null,diagnostics:[]})||{};
    const modelReadiness=engine.readiness||fallbackReadiness,quality=engine.quality||null,relativeEngine=engine.relative||null,reverse=engine.reverse||null;
    const intrinsicValuation=engine.valuation||null;
    const marketNormalizedFairValue=model==='operating-company'&&typeof core.jukaHistoricalMultipleFairValue==='function'
      ? safe('historical-multiple-fair-value',()=>core.jukaHistoricalMultipleFairValue(m.prices.map(x=>({date:x.date,price:x.close})),derived,m.asOf,stock.overrides||{}),null)
      : null;
    let valuation=marketNormalizedFairValue?.available
      ? {bear:marketNormalizedFairValue.bear,base:marketNormalizedFairValue.base,bull:marketNormalizedFairValue.bull,detail:{method:marketNormalizedFairValue.method,marketNormalizedFairValue,intrinsic:intrinsicValuation}}
      : intrinsicValuation;
    let relative=relativeEngine;
    const dataQuality=safe('data-quality',()=>core.jukaDataQuality(derived),null);
    const companyProfile=safe('company-profile',()=>core.jukaCompanyProfile(derived),null);
    const autoAssumptions=model==='operating-company'&&modelReadiness.ready?safe('auto-assumptions',()=>core.jukaAutoAssumptions(derived,stock.overrides||{}),null):null;
    const forecast=model==='operating-company'&&modelReadiness.ready?safe('forecast-5y',()=>core.jukaForecast5Y(derived,autoAssumptions?.assumptions||{}),null):null;
    const forecastScenarios=model==='operating-company'&&modelReadiness.ready?safe('forecast-scenarios',()=>core.jukaForecastScenarios(derived,autoAssumptions?.assumptions||{}),null):null;
    if(model==='operating-company'&&forecast?.summary?.epsY5){
      relative=safe('relative-valuation',()=>core.jukaRelativeByModel(model,{price,epsTtm:latest?.eps,epsY5:forecast.summary.epsY5,fairValue:valuation?.base}),relative);
    }
    const returnBridge=model==='operating-company'&&forecast?.summary?.epsY5?safe('return-bridge',()=>core.jukaReturnBridge({price,epsTtm:latest?.eps,epsY5:forecast.summary.epsY5,fairValue:valuation?.base}),null):null;
    const returnMatrix=model==='operating-company'&&forecastScenarios?safe('return-matrix',()=>core.jukaExpectedReturnMatrix({price,epsTtm:latest?.eps,scenarios:forecastScenarios,fairValues:{bear:valuation?.bear,base:valuation?.base,bull:valuation?.bull}}),null):null;
    const multiples=safe('valuation-multiples',()=>core.valuationMultiplesFromSnapshot({price,latest}),null);
    const historical=safe('historical-valuation',()=>core.buildHistoricalValuationSeries
    ...
    const historicalPlausibility=core.jukaHistoricalValuationAudit?safe('historical-plausibility',()=>core.jukaHistoricalValuationAudit(historical),null):null;
    const historicalIntegrity=core.jukaHistoricalIntegrityAudit?safe('historical-integrity',()=>core.jukaHistoricalIntegrityAudit(historical),null):null;
    ...
    const reality=safe('reality-check',()=>core.jukaRealityCheck({model,price,fairValue,qualityScore:quality?.score,relative,reverse}),null);
    const riskAudit=safe('risk-audit',()=>core.jukaRiskAudit({model,price,valuation,quality,readiness:modelReadiness,dataQuality,reverse}),null);
    const warnings=[];
    if(!m.prices.length)warnings.push(market?.warning||'Keine Marktdaten');
    ...
    if(diagnostics.length)warnings.push(`Interne Teilberechnung fehlgeschlagen: ${[...new Set(diagnostics.map(x=>x.stage))].join(', ')}`);
    ...
    const valuationStatus=valuation&&n(valuation.base)!==null?'ready':modelReadiness.ready?'calculation-unavailable':'missing-model-data';
    return {stock:{...stock},model,modelReadiness,valuationStatus,price,currency:m.currency,exchange:m.exchange,market:m,fundamentals:f,derived,latest,quality,valuation,intrinsicValuation,marketNormalizedFairValue,relative,reverse,dataQuality,companyProfile,autoAssumptions,forecast,forecastScenarios,returnBridge,returnMatrix,multiples,historical,historicalPlausibility,historicalIntegrity,fairValueIntegrity,reality,riskAudit,engineAssumptions:engine.assumptions||null,
      ...
      excessReturn:engine.excessReturn||null,engineDiagnostics:[...(engine.diagnostics||[]),...diagnostics],
      analysisStatus,warnings:[...new Set(warnings.filter(Boolean))],provenance:{market:{source:m.source,asOf:m.asOf,status:m.status,providerCode:market?.providerCode||null},fundamentals:{source:f.source,asOf:f.asOf,status:f.status,providerCode:f.providerCode}},
