(function(root,factory){
  ...
  function buildSnapshot(stock={},market={},fundamentals={},core){
    ...
    const modelReadiness=engine.readiness,quality=engine.quality,relativeEngine=engine.relative,reverse=engine.reverse;
    const intrinsicValuation=engine.valuation;
    const marketNormalizedFairValue=model==='operating-company'&&typeof core.jukaHistoricalMultipleFairValue==='function'
      ? core.jukaHistoricalMultipleFairValue(m.prices.map(x=>({date:x.date,price:x.close})),derived,m.asOf,stock.overrides||{})
      : null;
