...
async function alphaVantageFundamentals(stock,opts={}){
  ...
  if(iq.length===4&&bq.length){
    ...
    const explicitDebt=num(latestB.shortLongTermDebtTotal),debt=explicitDebt??((num(latestB.currentDebt)??0)+(num(latestB.longTermDebt)??num(latestB.longTermDebtNoncurrent)??0));
