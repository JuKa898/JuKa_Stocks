...
async function resolveAlphaSymbol(display,current,key){
  const ck=String(display||current||'').toUpperCase(),hit=resolvedSymbolCache.get(ck);if(hit)return hit;
  const u=new URL('https://www.alphavantage.co/query');u.searchParams.set('function','SYMBOL_SEARCH');u.searchParams.set('keywords',display||current);u.searchParams.set('apikey',key);
  const j=await fetchJson(u),matches=j.bestMatches||[],wanted=String(display||'').toUpperCase();
  const best=matches.find(x=>String(x['1. symbol']||'').toUpperCase().startsWith(wanted+'.'))||matches[0];
...
async function alphaVantageFundamentals(stock,opts={}){
  ...
  try{
    ...
    const fallback=await resolveAlphaSymbol(stock.s||stock.symbol,sym,key);
