...
async function secAdapter(stock){
  ...
  const r=await fetch(`https://data.sec.gov/api/xbrl/companyfacts/CIK${found.cik}.json`,{headers:{'User-Agent':UA(),'Accept-Encoding':'gzip, deflate'}});
...
async function marketAdapter(stock){
  ...
  const r=await fetch(u,{headers:{Authorization:`apikey ${key}`}}),j=await r.json();
